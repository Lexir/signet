...
public class ReplyWorkflowStarter {
    ...
    public ReplyWorkflowStarter(MailMembershipRepository memberships,
    ...
    public boolean startAiReply(UUID membershipId) {
        MailMembership m = memberships.findById(membershipId).orElse(null);
        if (m == null) {
            return false;
        }
        ...
        if (parsed == null) {
            log.warn("Не удалось прочитать письмо {} для генерации ответа", membershipId);
            return false;
        }
