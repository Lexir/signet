...
public class SettingsService {
    ...
    /** Интервал опроса почты в секундах (минимум 5, чтобы не долбить сервер). */
    @Transactional(readOnly = true)
    public int pollIntervalSeconds() {
        long value = parseLong(get(Keys.POLL_INTERVAL_SECONDS));
        if (value <= 0) {
            value = 45;
        }
        return (int) Math.max(5, value);
    }

    @Transactional
    public void savePollInterval(int seconds) {
        put(Keys.POLL_INTERVAL_SECONDS, String.valueOf(Math.max(5, seconds)), false);
    }
